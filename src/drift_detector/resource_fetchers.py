...
def get_live_aws_resources(state_data: Dict) -> Dict[str, Any]:
    ...
    for resource in state_data.get('resources', []):
        ...
        if resource_type.startswith('aws_instance'):
            ...
            live_resources.update(_fetch_dynamodb_tables(dynamodb_client, resource_name))
            ...
            live_resources.update(_fetch_eventbridge_buses(events_client, resource_name))
        elif resource_type.startswith('aws_cloudwatch_event_rule'):
            live_resources.update(_fetch_eventbridge_rules(events_client, resource_name))
            ...
            live_resources.update(_fetch_api_gateway_apis(apigateway_client, resource_name))
        elif resource_type.startswith('aws_cloudwatch_dashboard'):
            live_resources.update(_fetch_cloudwatch_dashboards(cloudwatch_client, resource_name))
        elif resource_type.startswith('aws_cloudwatch_metric_alarm'):
            live_resources.update(_fetch_cloudwatch_alarms(cloudwatch_client, resource_name))
...
def _fetch_ecs_clusters(ecs_client: Any, resource_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for cluster_arn in response['clusterArns']:
            ...
            live_resources[f"aws_ecs_cluster.{resource_name}"] = cluster_info['clusters'][0]
        return live_resources
    except Exception as e:
...
def _fetch_ecs_services(ecs_client: Any, resource_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for service_arn in response['serviceArns']:
            ...
            live_resources[f"aws_ecs_service.{resource_name}"] = service_info['services'][0]
        return live_resources
    except Exception as e:
...
def _fetch_api_gateway_apis(apigateway_client: Any, resource_name: str) -> Dict[str, Any]:
...
def _fetch_cloudwatch_dashboards(cloudwatch_client: Any, resource_name: str) -> Dict[str, Any]:
...
def _fetch_cloudwatch_alarms(cloudwatch_client: Any, resource_name: str) -> Dict[str, Any]:
